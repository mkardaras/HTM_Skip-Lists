...
static inline void *tx_thread_data_new(int tid,sl_intset_t *set,int seed,int seed2,int nb_threads)
{
...
static inline void tx_thread_percentages_print(void *thread_data)
{
  ...
  float commits,aborts,forced,conflict,capacity,expl,rest,lacqs;
  ...
  printf("%f  %% of attempted deletions hit dummy nodes\n",100*((float)data->dummy)/data->nb_remove);
...
static inline void tx_thread_data_add(void *d1, void *d2, void *dst)
{
	...
	dest->dummy=data1->dummy+data2->dummy;
...
static inline void add_dummy(void *thread_data)
{
	tx_thread_data_t *tdata = thread_data;
