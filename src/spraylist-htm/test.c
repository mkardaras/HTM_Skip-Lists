  ...
  while(running){
    if (unext){ // update
      if (turn==0){ //insert
        ...
        if (coarse_grain_transactions){
          tx_start(max_tx_retries, tx_data, &(set->global_lock));
          ret=spray(set,tx_data);
          tx_end(tx_data, &(set->global_lock));
        }
        else ret=spray(set,tx_data);
        if (ret) {
          turn=0;
          inc_removed(tx_data);
        }
...
int main(int argc, char **argv){
  ...
  int i,j,c,key,seed=DEFAULT_SEED,t_seed,t_seed2;
  ...
  if (seed == 0) srand((int)time(0));
    ...
    t_seed = rand();
    t_seed2 = rand();
    tx_data[i] = tx_thread_data_new(i,set,t_seed,t_seed2,nthreads);
  ...
  //printf("                   tid      #starts      #commits     #aborts(       TIMESTAMP    CONFLICT      CAPACITY      EXPLICIT      REST)        lacqs       END       LOST_KEY      DELETED\n");
	void *total_tx_data = tx_thread_data_new(-1,set,0,0,nthreads);
	for (i=0; i < nthreads; i++) {
