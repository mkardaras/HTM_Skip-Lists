  ...
  //           printf("%d %d\n", pval, deps[pval][0]);
        ...
        if (unext) { // update
          ...
          if (last < 0) { // add
            ...
            if (d->lin) {
              pval = val;
              if (insert(d->linden_set, pval, pval)){
                d->nb_added++;
                last = pval;
              }
            } else { // not linden
...
int main(int argc, char **argv)
{
  ...
  *running = 1;
...
#endif
      if (insert(linden_set, pval, pval)){
        last = pval;
        i++;
      }
    }
  ...
  if (lin) size = sequential_length(linden_set);
  else size = sl_set_size(set);
  printf("Set size     : %d\n", size);
  printf("Level max    : %d\n", *levelmax);
  ...
  *running = 0;
  ...
  if (lin) printf("Set size      : %d (expected: %d)\n", sequential_length(linden_set), size);
  else printf("Set size      : %d (expected: %d)\n", sl_set_size(set), size);
